...
class CrosswordCreator():
    ...
    def order_domain_values(self, var, assignment):
        ...
        """
        order =  list()
        for word in self.domains[var] - assignment.keys() :
            n = 0
            for neighbor in self.crossword.neighbors(var):
                if word in self.domains[neighbor]:
                    n += 1
            order.append((word, n))
        sorted(order, key= lambda word: word[1])
        order = [word[0] for word in order]
        return order
    ...
    def select_unassigned_variable(self, assignment):
        ...
        valid_vars = self.crossword.variables - set(assignment.keys())
        # Random minumum
        minimum = valid_vars.pop()

        # Select the variable with minimum values in its domain or largest degree
        for var in valid_vars:
            if len(self.domains[var]) < len(self.domains[minimum]):
                minimum = var
            elif len(self.domains[var]) == len(self.domains[minimum]):
                if len(self.crossword.neighbors(var)) > len(self.crossword.neighbors(minimum)):
                    minimum = var
